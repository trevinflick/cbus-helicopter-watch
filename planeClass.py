...
class Plane:
    ...
    def route_info(self):
        ...
        return route_to
    def _channel_enabled(self, section, circling=False):
        """Whether a notification channel should fire for the current dispatch. Circling
        dispatch checks ENABLE_CIRCLING first, falling back to ENABLE if that option is
        absent, so a channel can post circling events without also posting takeoff/landing."""
        if not self.config.has_section(section):
            return False
        if circling and self.config.has_option(section, 'ENABLE_CIRCLING'):
            return self.config.getboolean(section, 'ENABLE_CIRCLING')
        return self.config.has_option(section, 'ENABLE') and self.config.getboolean(section, 'ENABLE')
    def queue_bluesky_post(self, message, photo_path, db_event_id=None):
        """Holds a Bluesky post for BLUESKY.DELAY_MINS before sending, instead of posting in real time.
        db_event_id, if given, is a dataLog.py circling_events row id to record the post's
        AT URI against once it's actually sent."""
        import os
        import shutil
        import uuid
    ...
    def flush_bluesky_queue(self):
        ...
        for post in due:
            ...
            post_uri = sendBluesky(post['photo_path'], post['message'], self.config)
            if post.get('db_event_id') is not None:
                try:
                    import dataLog
                    dataLog.record_post_uri(post['db_event_id'], post_uri)
                except Exception as e:
                    print(f"Error recording post URI in event log, continuing: {e}")
